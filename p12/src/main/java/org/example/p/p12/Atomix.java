...
class Kanvas extends Canvas implements Runnable
{
	Atomix atomix;
	...
	public Kanvas(Atomix atomix)
	{
		this.atomix = atomix; 
		animovany = 0;
		HR = new Image("file:HR.png");
		HL = new Image("file:HL.png");
		HT = new Image("file:HT.png");
		HB = new Image("file:HB.png");
		OTB = new Image("file:OTB.png");
		OLR = new Image("file:OLR.png");
	...
	public void setStav(Stav_Hry hra)
	{
		atomix.hra = hra;
	}	
		...
		while (atomix.running)
		{
			...
			{				
				...
				if (atomix.hra.level < atomix.hra.maxLevel)
				{
					...
					atomix.hra = new Stav_Hry(atomix.hra.level + 1);
					Platform.runLater(new Runnable() { public void run() {
...
class Stav_Hry implements Serializable
{
	/**
	 * 
	 */
