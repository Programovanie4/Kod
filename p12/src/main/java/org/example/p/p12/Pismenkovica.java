...
public class Pismenkovica extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			running = true;
			hra = new StavHry1("pismenkovica1.txt");
	...
	private void load()
	{
		try { hra = StavHry1.precitaj(); } catch (Exception e) { e.printStackTrace(); }
		kanv.setStav(hra);
	}
...
class Kanva extends Canvas implements Runnable
{
	Pismenkovica app;
	StavHry1 hra;
	double sirkaPola, vyskaPola;
	
	public Kanva(Pismenkovica app, StavHry1 hra)
	{
		this.app = app;
		this.hra = hra;
		setW(500);
    ...
    public void setStav(StavHry1 s)
    {
    	hra = s;
    	app.aktualizujSkore();
    	app.aktualizujSlovo();
...
class StavHry1 implements Serializable
{
	...
	public StavHry1(String fileName)
	{
		nacitaj(fileName);
		skore = 0;
	}
    ...
    public static StavHry1 precitaj() throws Exception
    {
    	ObjectInputStream r = new ObjectInputStream(new FileInputStream("pismenkovica.bin"));
    	StavHry1 s = (StavHry1)r.readObject();
    	r.close();
