...
public class Mirrors extends Application {
	...
	static Boxx[][] Boxxes = new Boxx[SIZE + 2][SIZE + 2];
	...
	public void start(final Stage primaryStage) throws Exception {
		...
		for (int i = 0; i <= SIZE + 1; i++)
			for (int j = 0; j <= SIZE + 1; j++)
				gp.add(Boxxes[i][j] = new Boxx(this, i, j), j, i);
		// ---
...
class Boxx extends Canvas implements Serializable {
	/**
	 * 
	 */
	...
	public Boxx(Mirrors ap, int i, int j) {
		this.i = i;
		this.j = j;
	...
	*/
	private void composeColors(int i, int j, int shape, Light l) {
		if (l == null)
			return;
		if (Mirrors.Boxxes[i][j].shapes[shape] == null) {
			Mirrors.Boxxes[i][j].shapes[shape] = new MyColor(0, 0, 0);
		}
		Mirrors.Boxxes[i][j].shapes[shape].red += l.red;
		Mirrors.Boxxes[i][j].shapes[shape].green += l.green;
		Mirrors.Boxxes[i][j].shapes[shape].blue += l.blue;
	...
	public void paintAfter() {
		GraphicsContext gc = getGraphicsContext2D();
		gc.setLineWidth(3);
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.horiz] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.horiz].toColor());
			gc.strokeLine(0, getHeight() / 2, getWidth(), getHeight() / 2);
		}
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.verti] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.verti].toColor());
			gc.strokeLine(getWidth() / 2, 0, getWidth() / 2, getHeight());
		}
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.leftup] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.leftup].toColor());
			gc.strokeLine(0, getHeight() / 2, getWidth() / 2, getHeight() / 2);
			gc.strokeLine(getWidth() / 2, getHeight() / 2, getWidth() / 2, 0);
		}
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.rightdown] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.rightdown].toColor());
			gc.strokeLine(getWidth() / 2, getHeight(), getWidth() / 2, getHeight() / 2);
			gc.strokeLine(getWidth() / 2, getWidth() / 2, getWidth(), getHeight() / 2);
		}
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.leftdown] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.leftdown].toColor());
			gc.strokeLine(0, getHeight() / 2, getWidth() / 2, getHeight() / 2);
			gc.strokeLine(getWidth() / 2, getHeight() / 2, getWidth() / 2, getHeight());
		}
		if (Mirrors.Boxxes[i][j].shapes[Mirrors.rightup] != null) {
			gc.setStroke(Mirrors.Boxxes[i][j].shapes[Mirrors.rightup].toColor());
	...
	public void paint() {
		...
		for (Light l : Mirrors.lights) {
			...
			while (0 <= x && x <= Mirrors.SIZE + 1 && 0 <= y && y <= Mirrors.SIZE + 1) {
				int st = Mirrors.Boxxes[x][y].status;
				if (i == x && j == y) {
					if (st == 0 || st == -2 || st == 2) {
						if (dx == 0)
